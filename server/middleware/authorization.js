...
/**
 * Institute context middleware - simplified for single institute
 * @param {String} paramName - Request parameter name (kept for compatibility)
 */
const requireInstituteAccess = (paramName = 'instituteId') => {
  return async (req, res, next) => {
    try {
      ...
      // In single-institute mode, just continue
      // All users belong to PGC DHA campus
      next();
    } catch (error) {
      return res.status(500).json({
        success: false,
